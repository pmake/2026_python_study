...
def main():
    ...
    try:
        ...
        # CSS 定位選擇器（包含已轉義特殊字元如 [528px] 與小數點 .）
        css_selector = f"#main-layout > div > div.intro-section-right.product-purchase.goods-page-section > h1 > span"
        
        target_element = soup.select_one(css_selector)
        if target_element:
            print(f"找到目標元素標籤：{target_element}")
            print(f"目標文字內容：{target_element.text.strip()}")
        else:
