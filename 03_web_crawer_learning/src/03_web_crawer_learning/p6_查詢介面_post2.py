    ...
    try:
        ...
        for author_idx, author_name in enumerate(author_options, 1):
            ...
            for tag_name in tag_options:
                ...
                # 抓取頁面中對應的 "div.quote" 元素
                quote_elements = soup_search.select("div.quote")

                for quote_item in quote_elements:
                    # 依據 class 名稱精準擷取格言 (span.content)、作者 (span.author) 與標籤 (span.tag)
                    content_node = quote_item.select_one("span.content")
                    author_node = quote_item.select_one("span.author")
                    tag_node = quote_item.select_one("span.tag")

                    quote_text = content_node.get_text(strip=True) if content_node else ""
                    quote_author = author_node.get_text(strip=True) if author_node else ""
                    quote_tag = tag_node.get_text(strip=True) if tag_node else ""
        ...
        if all_quotes_data:
            excel_path = Path(__file__).parent / "data" / "quotes_search_data2.xlsx"
